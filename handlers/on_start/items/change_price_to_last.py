...
async def update_tracking_price(callback: CallbackQuery, user_product_service: UserProductService):
    number = callback.data.split('update_tracking_')[1]
    user_product_service.patch_start_price(callback.from_user.id, number)
