...
async def update_treshhold_to_n(callback: CallbackQuery, user_product_service: UserProductService):
    diff, percent, number = callback.data.split("_")
    user_id = callback.from_user.id
    if percent == 'always':
        user_product_service.patch_alert_threshold(user_id, number, 0)
        user_product = user_product_service.get_user_product_by_number(callback.from_user.id, number)
        ...
        user_product_service.patch_alert_threshold(user_id, number, percent)
        user_product = user_product_service.get_user_product_by_number(callback.from_user.id, number)
